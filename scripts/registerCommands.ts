...
import { allCommandApiInfo } from "../src/presentation/discord/apiInfo.ts";
...
async function registerSlashCommands(guild?: `${bigint}` | "user-install") {
    ...
    const info: RESTPostAPIChatInputApplicationCommandsJSONBody[] = allCommandApiInfo.map((apiInfo) =>
        getSlashCommandBuilder(apiInfo).toJSON(),
    );
