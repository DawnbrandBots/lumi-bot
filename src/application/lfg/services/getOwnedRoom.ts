...
export const getOwnedRoom: TLfgServiceBase<"getOwnedRoom", "persistence.lfg.findRoomByUser"> = async function (
    dependencies,
    { guildId, owner },
) {
    const room = await dependencies.persistence.lfg.findRoomByUser({ guildId, userId: owner.id });
    if (!room) {
