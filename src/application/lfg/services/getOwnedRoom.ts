...
export const getOwnedRoom: TLfgServiceBase<"getOwnedRoom", "repositories.lfg.findRoomByUser"> = async function (
    dependencies,
    { guildId, owner },
) {
    const room = await dependencies.repositories.lfg.findRoomByUser({ guildId, userId: owner.id });
    if (!room) {
