...
export function toAsciiTable(arg: { data: (string | number)[][], headerPadding?: number, cellPadding: number }) {
    ...
    function formatRow(row: (string | number)[]) { return rowToStr({ row, headerPadding, cellPadding: arg.cellPadding }) }
...
function rowToStr({ row, headerPadding, cellPadding }: { row: (string | number)[], headerPadding: number, cellPadding: number }) {
    return row[0]?.toString().padEnd(headerPadding, ' ') + "|" + row.slice(1).map(n => `${n}`.padStart(cellPadding, ' ')).join("  ")
}
