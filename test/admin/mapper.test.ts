...
describe(mapAdminFeatureReturnToMessage.name, () => {
    ...
    test("maps LFG config", () => {
        const message = mapAdminFeatureReturnToMessage({
            ...
            value: { guild: "guild-1", lfgChannel: CHANNEL_ID, lfgRole: ROLE_ID } as GuildConfig,
        });
        ...
        expect(message.embeds?.[0]).toMatchObject({
            ...
            fields: [
                ...
                { name: "Role", value: roleMention(ROLE_ID) },
            ],
        });
    });
    ...
    test("maps LFG role set", () => {
        ...
        expect(message.embeds?.[0]).toMatchObject({
            ...
            description: `LFG pings will mention ${roleMention(ROLE_ID)}.`,
        });
    });

    test("maps LFG role cleared", () => {
        const message = mapAdminFeatureReturnToMessage({ kind: EAdminFeatureReturnKind.LFG_ROLE_CLEARED });

        expect(message.kind).toBe(EMessageKind.POSITIVE);
        expect(message.embeds?.[0]).toMatchObject({ title: "LFG ping role cleared" });
    });
    ...
    test("maps invalid LFG channel options", () => {
        ...
        const missingRole = mapAdminFeatureReturnToMessage({
            kind: EAdminFeatureReturnKind.LFG_ROLE_MISSING_ROLE,
        });
        const invalidRoleOptions = mapAdminFeatureReturnToMessage({
